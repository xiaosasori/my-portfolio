<template>
  <header class="dark:bg-bdark sticky top-0 z-20 w-full bg-white border-b dark:border-gray-800 border-gray-300 bg-opacity-90">
    <div class="w-full py-4 mx-auto max-w-7xl">
      <div class="flex items-center justify-between px-10 lg:px-24">
        <nuxt-link to="/" class="text-2xl font-semibold dark:text-gray-200 dark:hover:text-tdark text-gray-900">
          MA
        </nuxt-link>
        <div class="hidden space-x-2 text-lg dark:text-gray-200 text-gray-900 md:flex">
          <nuxt-link
            v-for="nav in navs"
            :key="nav.name"
            class="px-4 py-2 font-semibold duration-200 rounded transition-background dark:hover:text-tdark"
            :to="nav.link"
          >
            {{ nav.name }}
          </nuxt-link>
        ...
        <div class="md:hidden">
          <button :aria-label="show ? 'close' : 'menu'" class="block dark:text-gray-300 text-gray-900 transition hover:opacity-50" @click="show = !show">
            <svg
              v-show="!show"
              class="w-6 h-6"
              fill="none"
...
export default {
  ...
  mounted () {
    if (this.darkMode) { document.documentElement.classList.toggle('dark', true) }
  },
  methods: {
    close () {
      this.show = false
